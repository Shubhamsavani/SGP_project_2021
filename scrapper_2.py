...
def get_details(url):
    ...
    temp_div=soup.find('div',class_='field field-name-field-credit field-type-list-text field-label-inline clearfix')
    audit=temp_div.find('div',class_='field')
...
for j in range(len(course_url)):
    reply=get_details(course_url[j])
...
all_dataframe.to_csv('Harvard_scrapped.csv',index=None)
